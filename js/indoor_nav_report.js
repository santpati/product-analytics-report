// Month band backgrounds for long daily trend charts (GA / Amplitude style)
        ...
        function updateTrendChartHints(view) {
            ...
            const qrHint = document.getElementById('qrTrendsHint');
            const visitorHint = document.getElementById('visitorTrendsHint');
            if (qrHint) qrHint.textContent = hint;
            if (visitorHint) visitorHint.textContent = hint;
            
            const isLongDaily = view === 'daily' && (getSelectedDateRange()?.days || 0) > 45;
            ['qrTrendsChartContainer', 'visitorTrendsChartContainer'].forEach(id => {
        ...
        function trackEvent(eventType, data = {}) {
            fetch('/api/analytics/track', {
                ...
                body: JSON.stringify({
                    event_type: '[IndoorNav] ' + eventType,
        ...
        function clearStaleReportData() {
            ...
            ['totalQRScans', 'uniqueVisitors', 'totalPOI', 'totalQuickAccess', 'totalSearches',
        ...
        async function fetchSites(accountId, startMs, endMs) {
            const sitesQuery = {
                response: { mimeType: 'application/json' },
                request: {
                    pipeline: [
                        { source: { trackEvents: { trackTypeId: QR_SCAN_TRACK_ID }, timeSeries: { period: 'dayRange', first: startMs, last: endMs } } },
                        { filter: `accountId == "${accountId}"` },
                        { group: { group: ['properties.locationHierarchy'], fields: [{ count: { sum: 'numEvents' } }] } },
                        { sort: ['-count'] }
                    ]
                }
            };
            const sitesData = await fetchAggregation(sitesQuery);
        ...
        function updateSitesDropdown(sites) {
            ...
            sites.forEach(site => {
                const location = site.properties?.locationHierarchy;
                if (location) {
                    const option = document.createElement('option');
                    option.value = location;
                    option.textContent = `${location} (${site.count})`;
        ...
        async function getData(options = {}) {
            ...
            showLoading(true, 'Fetching available sites...');
            ...
            try {
                const sites = await fetchSites(accountId, startMs, endMs);
                
                if (sites.length === 0) {
                    alert('No data found for this tenant ID and duration.');
                    showLoading(false);
                    return;
                }
                
                updateSitesDropdown(sites);
        ...
        async function loadData() {
            ...
            try {
                ...
                const [
                    ...
                    fetchAggregation(trackPipeline(QR_SCAN_TRACK_ID, { group: { fields: [{ count: { sum: 'numEvents' } }] } })),
                    fetchAggregation(trackPipeline(QR_SCAN_TRACK_ID, { group: { group: ['visitorId'], fields: [{ count: { count: null } }] } }, { limit: PENDO_AGGREGATION_LIMIT })),
                    fetchAggregation(trackPipeline(QR_SCAN_TRACK_ID, { group: { group: ['properties.mobileOs'], fields: [{ count: { sum: 'numEvents' } }] } })),
                    ...
                    fetchDailyEventTrends(QR_SCAN_TRACK_ID, filterExpr, dateRange),
                    fetchDailyVisitorTrends(QR_SCAN_TRACK_ID, filterExpr, dateRange)
                ]);
                if (isStale()) return;

                const totalScans = (totalData?.results?.[0]?.count) || 0;
                document.getElementById('totalQRScans').textContent = totalScans.toLocaleString();

                const uniqueVisitors = uniqueData?.results?.length || 0;
                document.getElementById('uniqueVisitors').textContent = uniqueVisitors.toLocaleString();

                rawDailyData = dailyData;
                loadedDailyPointCount = rawDailyData.length;
                renderTrendsChart(currentTrendsView);

                rawVisitorDailyData = visitorDailyData;
                renderVisitorTrendsChart(currentVisitorTrendsView);

                renderPlatformChart(platformData?.results || []);
        ...
        function renderPlatformChart(data) {
            ...
            data.forEach(d => {
                const os = (d.properties?.mobileOs || '').toLowerCase();
            ...
            pdfDiv.innerHTML = `
                <div style="font-family:Arial,sans-serif;padding:30px;color:#1a1a2e;width:750px;">
                    <div style="border-bottom:3px solid #0072ff;padding-bottom:15px;margin-bottom:20px;">
                        <div style="font-size:22px;font-weight:bold;color:#0072ff;">Cisco Spaces Indoor Navigation Analytics Report</div>
                    </div>
                    ...
                    <div style="margin-top:30px;padding-top:15px;border-top:1px solid #ddd;font-size:10px;color:#999;">Generated on ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })} | Cisco Spaces Indoor Navigation Analytics</div>
                </div>`;
...
<head>
    ...
    <title>Indoor Navigation Report - ${settings.customerName}</title>
...
<body>
    <div class="container">
        <div class="header">
            <div class="title">Cisco Spaces Indoor Navigation Analytics Report</div>
        ...
        <div class="footer">
            <div class="footer-text">
                ...
                Cisco Spaces Indoor Navigation Analytics
            </div>
